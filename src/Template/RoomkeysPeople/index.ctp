        ...
        <div class="table-responsive">
            <table class="table table-striped" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                        <?= $roomkeysPerson->has('roomkey') ? $this->Html->link($roomkeysPerson->roomkey->name,
                        ['controller' =>
                        'Roomkeys', 'action' => 'view', $roomkeysPerson->roomkey->id]) : '' ?>
                    </td>
                    <td>
                        <?= $roomkeysPerson->has('person') ? $this->Html->link($roomkeysPerson->person->full_name,
                        ['controller' =>
                        'People', 'action' => 'view', $roomkeysPerson->person->id]) : '' ?>
                    </td>
                    <td><?= h($roomkeysPerson->comment) ?></td>
                    <td class="actions">
                        <?= $this->Html->link('', ['action' => 'view', $roomkeysPerson->id], ['title' => __('View'),
                        'class' =>
                        'btn btn-default glyphicon glyphicon-eye-open']) ?>
                        <?= $this->Html->link('', ['action' => 'edit', $roomkeysPerson->id], ['title' => __('Edit'),
                        'class' =>
                        'btn btn-default glyphicon glyphicon-pencil']) ?>
                        <?= $this->Form->postLink('', ['action' => 'delete', $roomkeysPerson->id], ['confirm' => __('Are
                        you sure
                        you want to delete # {0}?', $roomkeysPerson->id), 'title' => __('Delete'), 'class' => 'btn
                        btn-default
