        ...
        <div class="table-responsive">
            <table class="table table-striped" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                        <?= $peopleUserRole->has('person') ? $this->Html->link($peopleUserRole->person->full_name,
                        ['controller' =>
                        'People', 'action' => 'view', $peopleUserRole->person->id]) : '' ?>
                    </td>
                    <td>
                        <?= $peopleUserRole->has('user_role') ? $this->Html->link($peopleUserRole->user_role->title,
                        ['controller' =>
                        'UserRoles', 'action' => 'view', $peopleUserRole->user_role->id]) : '' ?>
                    </td>
                    <td class="actions">
                        <?= $this->Html->link('', ['action' => 'view', $peopleUserRole->id], ['title' => __('View'),
                        'class' =>
                        'btn btn-default glyphicon glyphicon-eye-open']) ?>
                        <?= $this->Html->link('', ['action' => 'edit', $peopleUserRole->id], ['title' => __('Edit'),
                        'class' =>
                        'btn btn-default glyphicon glyphicon-pencil']) ?>
                        <?= $this->Form->postLink('', ['action' => 'delete', $peopleUserRole->id], ['confirm' => __('Are
                        you sure
                        you want to delete # {0}?', $peopleUserRole->id), 'title' => __('Delete'), 'class' => 'btn
                        btn-default
