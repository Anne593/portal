        ...
        <div class="table-responsive">
            <table class="table table-striped" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                        <?= $workingGroupsPerson->has('person') ?
                        $this->Html->link($workingGroupsPerson->person->full_name, ['controller' =>
                        'People', 'action' => 'view', $workingGroupsPerson->person->id]) : '' ?>
                    </td>
                    <td>
                        <?= $workingGroupsPerson->has('working_group') ?
                        $this->Html->link($workingGroupsPerson->working_group->name, ['controller' =>
                    ...
                    <td class="actions">
                        <?= $this->Html->link('', ['action' => 'view', $workingGroupsPerson->id], ['title' =>
                        __('View'), 'class' =>
                        'btn btn-default glyphicon glyphicon-eye-open']) ?>
                        <?= $this->Html->link('', ['action' => 'edit', $workingGroupsPerson->id], ['title' =>
                        __('Edit'), 'class' =>
                        'btn btn-default glyphicon glyphicon-pencil']) ?>
                        <?= $this->Form->postLink('', ['action' => 'delete', $workingGroupsPerson->id], ['confirm' =>
                        __('Are you sure
                        you want to delete # {0}?', $workingGroupsPerson->id), 'title' => __('Delete'), 'class' => 'btn
                        btn-default
